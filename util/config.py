...
class Configurable(object):
    ...
    TASK_CONFIG_TEMPLATE_VALUES = {
        'nmt-base':['nmt', NERDataset, {'index':'id', 'X':'text', 'y':'label'}, (['_nmt_transform'], [{}]), ([P._padtrim_transform], [{}]), {'ypad_val':'O', 'trimlbs':True, 'mdlcfg':{'maxlen':128}, 'ignored_label':'O'}],
        ...
        'sentsim-base': ['sentsim', SentSimDataset, {'index':'id', 'X':['sentence1','sentence2'], 'y':'score'}, ([], []), ([P._trim_transform, P._sentsim_transform, P._pad_transform], [{},{},{}]), {'binlb':'rgrsn', 'mdlcfg':{'sentsim_func':None, 'ymode':'sim', 'loss':'contrastive', 'maxlen':128}, 'ynormfunc':(lambda x: x, lambda x: x)}]
    }
    ...
    MODEL_CONFIG_TEMPLATE_VALUES = {
        'transformer-base': [P._base_encode, {'lm_loss':False, 'fchdim':0, 'iactvtn':'relu', 'oactvtn':'sigmoid', 'pdrop':0.2, 'do_norm':True, 'norm_type':'batch', 'do_extlin':False, 'do_lastdrop':True, 'do_crf':False, 'initln':False, 'initln_mean':0., 'initln_std':0.02, 'sample_weights':False}, (AdamW, {'correct_bias':False}, 'linwarm'), BaseShell],
        'nn-base': [P._tokenize, {'pooler':False, 'seq2seq':'isa', 'seq2vec':'boe', 'fchdim':768, 'iactvtn':'relu', 'oactvtn':'sigmoid', 'w2v_path':None, 'pdrop':0.2, 'do_norm':True, 'norm_type':'batch', 'do_lastdrop':True, 'do_crf':False, 'initln':False, 'initln_mean':0., 'initln_std':0.02, 'sample_weights':False}, None, BaseShell]
    }
    ...
    PREDEFINED_MODEL_CONFIG_VALUES = {
        'bert': ['transformer-base', 'bert-base-uncased', BertModel, BertConfig, 'BERT', BERTClfHead, BertTokenizer, ['[CLS]', '[SEP]', '[SEP]', '[MASK]'], {}],
        'gpt': ['transformer-base', 'openai-gpt', OpenAIGPTLMHeadModel, OpenAIGPTConfig, 'GPT', GPTClfHead, OpenAIGPTTokenizer, ['_@_', ' _$_', ' _#_'], {}],
        'gpt2': ['transformer-base', 'gpt2', GPT2LMHeadModel, GPT2Config, 'GPT-2', GPTClfHead, GPT2Tokenizer, ['_@_', ' _$_', ' _#_'], {}],
        'trsfmxl': ['transformer-base', 'transfo-xl-wt103', TransfoXLLMHeadModel, TransfoXLConfig, 'TransformXL', TransformXLClfHead, TransfoXLTokenizer, ['_@_', ' _$_', ' _#_'], {}],
        'elmo': ['nn-base', 'elmo', Elmo, elmo_config, 'ELMo', {'pool':EmbeddingPool, 's2v':EmbeddingSeq2Vec, 's2s':EmbeddingSeq2Seq, 'ss2v':SentVecEmbeddingSeq2Vec}, None, None, {'embed_type':'elmo'}]
    }
    # Common parameters
